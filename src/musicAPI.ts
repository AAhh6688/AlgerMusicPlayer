...
export const musicAPI = {
  // 网易云搜索（自动切换备选 API）
  async searchNetease(keyword: string, limit = 20) {
  ...
  // QQ 音乐搜索（自动切换备选 API）
  async searchQQ(keyword: string, limit = 20) {
  ...
  // 酷狗搜索（自动切换备选 API）
  async searchKugou(keyword: string, limit = 20) {
  ...
  // 酷我搜索（自动切换备选 API）
  async searchKuwo(keyword: string, limit = 20) {
  ...
  // 获取歌曲 URL（网易云）
  async getNeteaseUrl(songId: string) {
  ...
  // 获取歌词（网易云）
  async getNeteaseLyric(songId: string) {
    for (const api of MUSIC_APIS.netease) {
      try {
        const response = await fetch(
          `${api}/lyric?id=${songId}`,
